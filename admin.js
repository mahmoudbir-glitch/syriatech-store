...
$("#uploadBtn").onclick=async()=>{
 ...
 try{
  ...
  const prep=await api("presign",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({filename:f.name,contentType:f.type,size:f.size})});
  const putResponse=await fetch(prep.presignedUrl,{method:"PUT",body:f,headers:{"Content-Type":f.type}});
  if(!putResponse.ok) throw new Error("فشل إرسال الصورة إلى Vercel Blob ("+putResponse.status+")");
  const imageUrl="/api/image?pathname="+encodeURIComponent(prep.pathname);$("#image").value=imageUrl;$("#preview").src=imageUrl;$("#preview").hidden=false;$("#saveMsg").textContent="تم رفع الصورة بنجاح ✓";
